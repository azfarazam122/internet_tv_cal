...
function assigningValuesToAllInputFields(data) {
	...
	if (data.currenttabThatsOpen_Json == 1) {
		...
		forSelectingABox("directv_AutoPay", data.direcTv_autoPay_SelectedValue_Json);
		runOperation_Tab6();
		...
		if (data["i3Broadband_internetOptionsSlider_SelectedValue_Json"] == "0mbps") {
			slider17.update({
				from: "0",
			});
		} else if (
		...
		if (data["centuryLink_internetOptionsSlider_SelectedValue_Json"] == "0mbps") {
			slider24.update({
				from: "0",
			});
		} else if (
		...
		} else if (data["rcn_internetOptionsSlider_SelectedValue_Json"] == "50mbps") {
			slider26.update({
				from: "1",
			});
		} else if (
		...
		var slider28 = $("#at_t_dtv_Internet").data("ionRangeSlider");
		...
		runOperation_Tab13();
	} else if (data.currenttabThatsOpen_Json == 14) {
		// COMCAST
		forSelectingABox(
			"dtv_channelLineup_checkboxes",
			data.dtv_channelLineup_SelectedValue_Json
		);

		var slider29 = $("#dtv_tv").data("ionRangeSlider");
		slider29.update({
			from: data["dtv_tvSlider_SelectedValue_Json"],
		});

		forSelectingABox(
			"dtv_EquipmentOptions_checkboxes",
			data.dtv_equipmentOptions_SelectedValue_Json
		);

		forSelectingMultipleBoxes(
			"dtv_PremiumChannels_checkboxes",
			data.dtv_premiumOptions_SelectedValue_Json
		);
		forSelectingABox("dtv_modernRental", data.dtv_modemRental_SelectedValue_Json);
		var slider30 = $("#dtv_Internet").data("ionRangeSlider");
		if (data["dtv_internetOptionsSlider_SelectedValue_Json"] == "0mbps") {
			slider30.update({
				from: "0",
			});
		} else if (
			data["dtv_internetOptionsSlider_SelectedValue_Json"] == "300mbps"
		) {
			slider30.update({
				from: "1",
			});
		} else if (
			data["dtv_internetOptionsSlider_SelectedValue_Json"] == "500mbps"
		) {
			slider30.update({
				from: "2",
			});
		} else if (
			data["dtv_internetOptionsSlider_SelectedValue_Json"] == "1000mbps"
		) {
			slider30.update({
				from: "3",
			});
		...
		forSelectingABox(
			"dtv_AddPhone_checkboxes",
			data.dtv_addPhone_SelectedValue_Json
		);
		// dtv_addPhone_SelectedValue_Json;
		forSelectingABox(
			"dtv_activationFee_checkboxes",
			data.dtv_activationFee_SelectedValue_Json
		);
